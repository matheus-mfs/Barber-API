...
from schemas import UserResponseSchema, UserSchema, LoginSchema
...
@router.post("/create_account", response_model=List[UserResponseSchema])
def create_account(user_schema: UserSchema, session: Session = Depends(get_session), current_tenant: Tenant = Depends(get_tenant)):
    
    user = create_account_service(session, user_schema, current_tenant)
    return user
